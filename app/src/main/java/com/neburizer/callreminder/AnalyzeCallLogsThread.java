 ...
 */
public class AnalyzeCallLogsThread implements Runnable{
    Context cxt;
                ...
                for(int j=i+1;j<calledArray.length;j++)
                {
                    ...
                    for(int iSkipDays=1;iSkipDays<=skipDays;iSkipDays++) {
                        if (timeDiff < ((long) 86400000*iSkipDays + skipMillis)) {   //condition if timeDiff lies between (skipdays*24 +/- skipHours)
                            ...
                            if(timeDiff > ((long) 86400000*skipDays + skipMillis)) break;  //maximum skip time-interval exceeded
                        }
                    }
                    if(found == 1 )break;        //break iteration if repeatCount>minRepeatCount
                }//[C] - loop through subarray of called list timings starting from i+1
            }//[B] - loop through all called list timings
        }//[A] - loop through each phone number
       sendMsg("\n-----Reminders------");
    ...
    private void sendMsg(String msg){
        ...
        bundle.putString(opKey,msg);
        Message message = MainActivity.btnHandler.obtainMessage();
        message.setData(bundle);
        MainActivity.btnHandler.sendMessage(message);
    }
}
